...
export function DeleteCategoryForm({id,hardDelete = false}: {id: string;hardDelete?: boolean;}) {
  const [open, setOpen] = useState(false);

  const mutation = useMutation({
    mutationFn: (categpryId: string) => {
      return categoriesService().deleteApiCategoriesClient({ id: categpryId });
    },
    onSuccess: (data) => {
      if (data.type === "error") {
        makeHotToast({
      ...
      makeHotToast({
        ...
        description: `Category has been ${hardDelete ? "permanently" : ""} deleted successfully`,
        variant: "success",
      });
      setOpen(false);
    },
  ...
  return (
    <DiaDrawer
      ...
      description={`${hardDelete ? "Permanently" : ""} Delete category item`}
      trigger={
        hardDelete ? (
          <button className="btn btn-outline btn-error btn-sm flex items-center justify-center gap-2">
